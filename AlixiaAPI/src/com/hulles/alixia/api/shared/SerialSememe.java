...
/**
 * Sememe is an important class in Alixia. It represents an object that causes an action of some
 * kind to take place: a question that prompts an answer, a command that prompts a response, and
 * so forth.
 *  <p>
 *  SerialSememe is a pared-down version of the main (non-API) Alixia Sememe. 
 *  
 * @author hulles
 *
 */
public class SerialSememe implements Serializable, Comparable<SerialSememe> {
    ...
    public static SerialSememe possiblyFind(String name) {
    	...
    	if (sememeSet == null) {
    		throw new AlixiaException("SerialSememe: sememe set not loaded");
    	}
    ...
    /**
     * Find a sememe in the set of all sememes. If the name is not found, throw an {@link AlixiaException}.
    ...
    public static SerialSememe find(String name) {
    	...
    	if (sememe == null) {
    		throw new AlixiaException("Unable to find sememe named " + name);
    	}
    	return sememe;
    }
