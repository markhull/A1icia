...
final public class DocumentSampleGenerator {
	...
	public List<DocumentSample> getSamples() {
        ...
        try {
            ...
            throw new AlixiaException("Can't create affirmatives URL", ex);
        }
		...
		try (BufferedReader dataStream = new BufferedReader(new InputStreamReader(textURL.openStream()))) {
			...
			throw new AlixiaException("Can't retrieve affirmative data", e);
		}
		
        try {
            textURL = new URL(openNLPPath + "/negatives.txt");
        } catch (MalformedURLException ex) {
            throw new AlixiaException("Can't create negatives URL", ex);
        }
		try (BufferedReader dataStream = new BufferedReader(new InputStreamReader(textURL.openStream()))) {
			while ((line = dataStream.readLine()) != null) {
			...
			throw new AlixiaException("Can't retrieve negative data", e);
		}
