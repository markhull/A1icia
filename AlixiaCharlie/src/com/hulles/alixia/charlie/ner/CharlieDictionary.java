 ...
 *******************************************************************************/
package com.hulles.alixia.charlie.ner;
	...
	public CharlieDictionary(DictionaryType type, TokenizerME tokenizer) {
		...
		switch (type) {
			...
			case ORGANIZATION:
				...
				tokens = tokenizer.tokenize("Alixia Death Commandos");
				orgList = new StringList(tokens);
				orgDictionary.put(orgList);
				if (tokens.length > maxTokens) {
					maxTokens = tokens.length;
				...
				break;
			default:
				throw new AlixiaException("CharlieDictionary: unsupported dictionary type = " + type.name());
		}
	}
	...
	public boolean contains(StringList tokens) {
		...
		switch (dictionaryType) {
			...
			case LOCATION:
				...
				return (cityCount > 0);
			default:
				throw new AlixiaException("CharlieDictionary: unsupported dictionary type = " + 
						dictionaryType.name());
		}
	}
