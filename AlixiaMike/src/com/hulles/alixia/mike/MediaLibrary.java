...
public class MediaLibrary {
	private final static Logger LOGGER = Logger.getLogger("AlixiaMike.MediaLibrary");
...
//	private final static Level LOGLEVEL = Level.INFO;
	...
	private void updateMediaItem(Path filePath) {
		...
		if (mediaFile == null) {
			...
			if (fileName.endsWith("mp4")) {
				...
				AlixiaUtils.error("MikeRoom:updateMediaItem bad media file extension");
				return;
			}
    ...
    TikaResult getMetaData(Path path) {
        ...
        if (artist == null || title == null) {
            AlixiaUtils.error("MediaLibrary: artist and/or title missing for file " + path);
            return null;
        }
