...
public class ImportExport {
	...
	private static void storeStation(Station station) {
		...
		stationKeyBytes = JebusBible.getBytesKey(JebusKey.ALIXIASTATIONKEY, jebusPool);
        jsonStationKeys = createJSONKeys(station);
		jsonKey = JebusBible.getStringKey(JebusKey.ALIXIAJSONSTATIONKEY, jebusPool);
		try {
			stationBytes = Serialization.serialize(station);
		} catch (IOException e) {
			throw new AlixiaException("ImportExport: can't serialize station", e);
		}
