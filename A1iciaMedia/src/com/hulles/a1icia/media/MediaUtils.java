...
public class MediaUtils {
	private final static Logger LOGGER = Logger.getLogger("A1iciaMedia.MediaUtils");
    ...
    private final static String MPVOPT1 = "--title=A1iciaVision";
	...
	public static void byteArrayToFile(byte[] bytes, String path) {
		...
		try {
			...
			throw new A1iciaMediaException("MediaUtils: can't convert byte array to file", e);
		}
	}
	...
	public static int statCommand(String[] cmd) {
		...
		try {
			...
			throw new A1iciaMediaException("MediaUtils: can't start statCommand process", e);
		}
		...
		try {
			...
			throw new A1iciaMediaException("MediaUtils: statCommand interrupted", e);
		}
		
		try {
			retVal = proc.exitValue();
		} catch (IllegalThreadStateException e) {
			throw new A1iciaMediaException("MediaUtils: statCommand not terminated when queried", e);
		}
		return retVal;
	}
