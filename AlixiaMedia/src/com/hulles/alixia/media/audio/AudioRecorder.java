...
//        int chunkBytes = (int) ((CHUNKFACTOR * sampleRateHz) * channels * bytesPerSample);
        ...
        try {
			...
			throw new AlixiaMediaException("AudioRecorder: can't run " + RECORDEXEC, ex);
        } finally {
	        try {
				...
				throw new AlixiaMediaException("AudioRecorder: can't delete temp file", ex);
	        }
	    }
    ...
    private static String sampleWidthToString(int width) {
    	...
    	switch(width) {
	    	...
	    	case 4:
	    		return "s32";
    		default:
				throw new AlixiaMediaException("AudioRecorder: bad sample width = " + width);
    	}
    }
...
//		StringBuilder stdOut;
    	...
    	try {
			...
			throw new AlixiaMediaException("AudioRecorder: can't create temp file", e1);
		}
		...
		try {
			...
			throw new AlixiaMediaException("AudioRecorder: can't start statCommand process", e);
		}
...
/*		stdOut = new StringBuilder();
		...
		try (BufferedReader inStream = new BufferedReader(
			...
			throw new AlixiaMediaException("AudioRecorder: can't read runCommand stream", e);
		}
...
*/
		...
		try {
			...
			throw new AlixiaMediaException("AudioRecorder: statCommand interrupted", e);
		}
		
		try {
			retVal = proc.exitValue();
		} catch (IllegalThreadStateException e) {
			throw new AlixiaMediaException("AudioRecorder: statCommand not terminated when queried", e);
		}
		
		if (retVal != 0) {
			throw new AlixiaMediaException("AudioRecorder: statCommand terminated with non-zero ret val = " + retVal);
		}
