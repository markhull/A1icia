...
final public class AlixiaMediaServlet extends HttpServlet {
	private final static Logger LOGGER = Logger.getLogger("AlixiaWeb.AlixiaMediaServlet");
	...
	public AlixiaMediaServlet()  {

        LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: instantiated");
	}
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException	  {
	    ...
	    if (imgParam != null) {
            imageHits++;
			LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: image query = {0}, hits = {1}", 
	    ...
	    if (mmdParam != null) {
            mediaHits++;
			LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: multimedia query = {0}, hits = {1}", 
	...
	private static boolean writeAudio(HttpServletResponse response, MediaFormat format, byte[] audioBytes) {
		...
		LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: preparing audio for response");
	...
	private static boolean writeAudio(OutputStream stream, byte[] audioBytes) {
		...
		LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: writing audio into response, bytes len = {0}", audioBytes.length);
	    try {
...
//				logger.log(LOGLEVEL, "AlixiaMediaServlet serving image for = " + 
...
//	    return image;
	...
	public static Long saveAudioBytes(MediaFormat format, byte[] audioBytes) {
		...
		LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: in saveAudioBytes, bytes len = {0}", audioBytes.length);
	...
	private static byte[] getAudioBytes(String key) {
		...
		LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: in getAudioBytes");
		jebusPool = JebusHub.getJebusLocal();
		...
		LOGGER.log(LOGLEVEL, "AlixiaMediaServlet: leaving getAudioBytes, bytes len = {0}", audioBytes.length);
		return audioBytes;
	}
